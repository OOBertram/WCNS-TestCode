subroutine solutiontime_RK3_WCNS_2DRT(index)
use input; use common1; use scheme_WCNS
integer nt(4),num_uns,num,num_check
real rhs(i1,j1,k1,5),timenowmin,distance(3),vel(3),time_FT,time_FT1,timeuse
...
do i=1,4
  nt(i)=0
  timenow1(i)=timebegin
enddo
timenowmin=timebegin
le=1
do while (timenowmin<timeend) 
if(le <= 3)then
  th=min(cfl/eigenmax(le),timeend,abs(timeend-timenow1(le)))
...
!***************!
    ...
    th=min(cfl/eigenmax(1),cfl/eigenmax(2),cfl/eigenmax(3),abs(timeend-timenow1(4)))
    call solutionsource(th)
endif
select case(le)
case(1)
...
case(3)
  ...
  timenow1(le)=timenow1(le)+th
  nt(le)=nt(le)+1
  write(*,*) le,nt(le),timenow1(le),th,vel(1),vel(2),distance(1)/((xma-xmi)/Dble(imax1)), &
  distance(2)/((yma-ymi)/Dble(jmax1)),eigenmax(le)
  timenowmin=timenowminimum1(timenow1,le)	
  vel(:) = 0.0
  enddo 
